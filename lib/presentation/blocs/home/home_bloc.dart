...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  Future<void> _onLoadHomeData(
    ...
    try {
      ...
      final firestoreGroups = await _categoryRepository.fetchCategories();
      
      // Convert from CategoryGroupFirestore to CategoryGroup
      final categoryGroups = _convertCategories(firestoreGroups);
  ...
  // Convert CategoryGroupFirestore list to CategoryGroup list
  List<CategoryGroup> _convertCategories(List<CategoryGroupFirestore> firestoreGroups) {
    return firestoreGroups.map((firestore) {
      // Convert items
      final items = firestore.items.map((item) => CategoryItem(
        id: item.id,
        label: item.label,
      ...
      // Create CategoryGroup with padding if needed
      // CategoryGroup requires exactly 8 items
      while (items.length < 8) {
        items.add(CategoryItem(
          id: 'empty_${items.length}',
          label: '',
          imagePath: '', // Empty image path
        ));
      }
      
      // Ensure we only use exactly 8 items
      final finalItems = items.take(8).toList();
      
      return CategoryGroup(
        id: firestore.id,
        title: firestore.title,
        backgroundColor: firestore.backgroundColor,
        itemBackgroundColor: firestore.itemBackgroundColor,
        items: finalItems,
      );
    }).toList();
  }
